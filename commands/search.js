...
module.exports = {
    ...
    run: async function (client, message, args) {
        ...
        if (!searchString)
            return sendError("You didn't poivide want i want to search", message.channel);
        ...
        try {
            ...
            try {
                ...
                console.error(err);
                return sendError("Nothing has been selected within 20 seconds, the request has been canceled.", message.channel);
            }
            const videoIndex = parseInt(response.first().content);
            var video = await searched[videoIndex - 1];
        } catch (err) {
            console.error(err);
            return sendError("🆘  **|**  I could not obtain any search results", message.channel);
        }
