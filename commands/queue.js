...
module.exports = {
  ...
  run: async function (client, message, args) {
    ...
    try {
      ...
      console.error(error);
      sendError(error.message).catch(console.error);
    }
    ...
    collector.on("collect", async (reaction, user) => {
      try {
        ...
        console.error(error);
        return sendError(error.message).catch(console.error);
      }
    });
    collector.on("end", () => {
      queueEmbed.reactions.removeAll().catch(console.error);
      if (queueEmbed && !queueEmbed.deleted) {
...
function generateQueueEmbed(message, queue) {
  ...
  for (let i = 0; i < queue.length; i += 10) {
    ...
    if(serverQueue.songs.length === 1)
      embed.setDescription(`No songs to play next add songs by \`\`${message.client.config.prefix}play <song_name>\`\``)
